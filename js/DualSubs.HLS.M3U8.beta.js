...
async function setOptions(Platform = "", Json = {}, Languages1 = [], Languages2 = [], Types = [], Standard = true, Type = "") {
	...
	async function getOptions(platform = "", obj1 = {}, obj2 = {}, types = [], standard) {
		...
		return types.map(type => {
			...
			// 修改语言代码
			newSub.OPTION.LANGUAGE = (standard) ? `\"${obj1.Language}\"` : (!standard) ? `\"${obj2.Language}\"`
				: (platform == "Disney_Plus" || platform == "Hulu") ? `\"${obj1.Language} ${obj2.Language} ${type}\"`
					: `\"${obj2.Language}\"`
			// 增加副语言
...
async function isStandard(platform, url, headers) {
    ...
    let standard = true;
    if (platform == "HBO_Max") {
        if (headers?.["User-Agent"]?.includes("Mozilla/")) standard = false;
